...
export default function FooterSection() {
  return (
    ...
    >
      ...
      >
        ...
        >
          ...
          <div className="self-stretch flex justify-start items-center gap-3">
            <motion.div 
              className="text-center text-white text-xl font-semibold leading-4 font-sans"
        ...
        >
          ...
          >
            ...
            <div className="flex flex-col justify-end items-start gap-2">
              {["Features", "Pricing", "Integrations", "Real-time Previews", "Multi-Agent Coding"].map((item) => (
                <motion.div 
                  key={item}
                  className="text-gray-400 text-sm font-normal leading-5 font-sans cursor-pointer hover:text-white transition-colors"
                  whileHover={{ x: 5 }}
                  transition={{ type: "spring", stiffness: 400 }}
                >
                  {item}
          ...
          >
            ...
            <div className="flex flex-col justify-center items-center gap-2">
              {["Terms of use", "API Reference", "Documentation", "Community", "Support"].map((item) => (
                <motion.div 
                  key={item}
                  className="self-stretch text-gray-400 text-sm font-normal leading-5 font-sans cursor-pointer hover:text-white transition-colors"
                  whileHover={{ x: 5 }}
                  transition={{ type: "spring", stiffness: 400 }}
                >
                  {item}
      ...
      <div className="self-stretch h-12 relative overflow-hidden border-t border-b border-gray-800">
        <div className="absolute inset-0 w-full h-full overflow-hidden">
          <div className="w-full h-full relative">
            {Array.from({ length: 400 }).map((_, i) => (
              <div
                key={i}
                className="absolute w-[300px] h-16 border border-gray-800"
                style={{
                  left: `${i * 300 - 600}px`,
                  top: "-120px",
